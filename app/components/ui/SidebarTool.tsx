    ...
    ({ icon, label, isActive, onClick, badge, badgeCount, disabled, popover }, ref) => {
        ...
        const ToolButton = (
            <button
                ...
                disabled={disabled}
                className={`relative flex flex-col items-center gap-1.5 transition-all duration-200 w-full 
            ...
            >
                ...
                <span
                    className={`flex items-center justify-center p-3 squircle-element transition-all duration-200 relative
                        ${!disabled && isActive ? "" : "hover:bg-white/5"}
                    `}
                ...
                >
                    <div className={`transition-transform duration-300 ${isActive && !disabled ? "scale-110" : !disabled ? "hover:scale-105" : ""}`}>
                        {typeof icon === "string" ? (
                ...
                >
                    <div className="relative w-full aspect-3/4 flex flex-col justify-end overflow-hidden bg-[#0c0c0e]">
                        ...
                        <div className="absolute inset-0 z-0">
                            <img
                                src="/images/pages/thumbnail-popover.avif"
                                className="w-full h-full object-cover blur-3xl opacity-40 scale-110"
                                alt="thumbnail-popover"
                            />
                        ...
                        {isOpen && (
                            <video
                                ...
                                muted
                                playsInline
                                className="absolute inset-0 w-full h-full object-cover mask-b-to-90% mask-b-from-90% z-10"
                            />
                        )}

                        <div className="absolute inset-0 z-20 bg-linear-to-t from-[#09090B] via-[#09090B]/60 to-transparent pointer-events-none" />

                        <div className="relative z-30 p-5">
                            <h3 className="text-sm font-bold text-white mb-1 drop-shadow-md">
                                {popover.title}
                            </h3>
                            <p className="text-[11px] text-cyan-50/50 leading-relaxed drop-shadow-md">
                                {popover.description}
                            </p>
                        </div>

                        <div className="absolute top-0 inset-x-0 h-px bg-linear-to-r from-transparent via-cyan-400/50 to-transparent z-40" />
                    </div>
                </PopoverContent>
